export function CalLinearSpline(points, targetX) {
	const x = points.map((point) => (parseFloat(point.x)));
	const y = points.map((point) => (parseFloat(point.fx)));
...
export function CalQuadraticSpline(points, targetX) {
	const pointX = points.map((point) => (parseFloat(point.x)));
	const pointY = points.map((point) => (parseFloat(point.fx)));
...
export function CalCubicSpline(points, targetX) {
	const x = points.map((point) => (parseFloat(point.x)));
	const y = points.map((point) => (parseFloat(point.fx)));
